  ...
  return (
    <>
      ...
      <div>
        ...
        <Button
          text="neutral"
          valueSetter={() => setNeutralCount(neutralCount + 1)}
        />
...
const Statistics = ({ goodCount, neutralCount, badCount }) => {
  return (
    <table>
      ...
      <StatisticLine
        text="average"
        value={(goodCount - badCount) / (goodCount + badCount + neutralCount)}
      />
      <StatisticLine
        text="positive"
        value={(goodCount / (goodCount + badCount + neutralCount)) * 100}
      />
    </table>
  );
};
...
const Button = ({ text, valueSetter }) => {
  return <button onClick={valueSetter}>{text}</button>;
};
